...
const createSet = (parentNode, input) =>
  forNode(parentNode.node('offer'), input, (set, key, entry) => {
    switch (key) {
      case 'id':
        set.att(key, entry)
        break
      default: set.node(key).txt(entry)
    }
  })
...
const createYML = (input, options) => {
  ...
  forNode(yml, input, (yml, key, entry) => {
    switch (key) {
      ...
      case 'sets':
        forNodeEach(yml.node(key), entry, (sets, entry) => {
          createSet(sets, entry)
        })
        break
